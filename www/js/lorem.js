...
var buildLorem = function (loremWords, num) {
	...
	(num > 7)? finalWords = punctuate(wordCollection): finalWords = wordCollection;
...
var isNotDiv = function (elem) {
	...
	for (var i = 0; i < classes.length; i++) {
		...
		// Else if it is a number.
			...
			elem.innerHTML = buildLorem(loremWords, Number(classes[i]) + 1);
			break;
		}	
...
var isDiv = function (elem) {
	...
	if (numWords > 0 && pgraphs > 0) {
		...
		for (var i = 0; i < pgraphs; i++) {
			// buildLorem(loremWords, <number of words>);
			elem.innerHTML += "<p>" + buildLorem(loremWords, numWords) + "</p>";
		}
	}
	// After completing iterations and only number of paragraphs are specified.
		...
		for (var i = 0; i < pgraphs; i++) {
			// buildLorem(loremWords, <number of words>);
			elem.innerHTML += "<p>" + buildLorem(loremWords, pickRandom([5, 6, 7, 8])) + "</p>";
		}
	}
